...
def MD5_Calculate(InputFolder):
    ...
    if f_hashed > 0:
        ...
        md5log += f"{round(md5log_minsize, 5)}\n"
        md5log += f"{round(md5log_maxsize, 5)}\n"
        md5log += "MD5LOG"
        ...
        while True:
            ...
            if founded_log_file == "*отмена": break
                ...
                try:
                    ...
                    print(f"    $ {ORANGE}Сохранение файла..{ENDC}", end='')
                    founded_log_file += '.md'
...
def showhelp():
    print(f""" $ {ORANGE}Справка по использованию{ENDC}
  (1)Составить контрольные суммы файлов каталога :
     Опции:
     1. Ввести каталог.
     2. *имя1*имя2*имя3 — ввести имена игнорируемых каталогов в пути к файлам (чтобы ввести слова из пунктов ниже вводите две звёздочки в начале (Пример: **назад)).
     3. >число  — Установить игнорирование файлов размером более указанного числа.
     4. <число  — Установить игнорирование файлов размером менее указанного числа.
     5. *очистить — Очистить игнорируемые размеры файлов и список игнорируемых каталогов.
     6. *имена  — Показывать и записывать в файл  Только имена  около контрольной суммы.
     7. *полные — Показывать и записывать в файл  Полные пути   около контрольной суммы.
     8. *назад  — Вернуться к выбору.

     9  (При сохранении файла с MD5 данными). *отмена — Вернуться к вводу каталога.
     10 (При сохранении файла с MD5 данными). Ввести имя файла/полный путь + имя файла и сохранить файл с MD5 данными для применения в Опции 2 для поиска/удаления копий (Пример: C:\\UserDesktop\\md5.txt  ->  Файл "md5.txt").
     11 (При сохранении файла с MD5 данными). Ввести каталог и поставить вопросительный знак в самом начале (Пример: ?C:\\UserDesktop\\md5  ->  Файл "md5.md"), чтобы сохранить Markdown-файл с контрольными суммами для чтения.

  (2)Составить контрольные суммы файлов каталога :
     Опции:
     1. 1-9 пункты из Опции 1.
     2. *allfound — Вкл/Выкл заканчивать поиск, когда количество найденных файлов равняется количеству объектов из файла с MD5 данными.
     3. *copydel  — Вкл/Выкл режим удаления копий объектов из импортированного файла с MD5 данными.
     4 (При вводе файла с MD5 данными). Пример: C:\\UserDesktop\\md5.txt (Из примера Опции 1). Принимает только файлы составленные Опцией 1. Про содержимое файла следующий пункт справки.

     5 (При сохранении Markdown-лога найденных файлов). *отмена — вернуться к вводу каталога.
          
   3. Содержимое файла с MD5 данными :
     Образец содержимого:
       Путь к файлу (или имя файла (или что угодно — имя объекта))
       MD5 файла
       Минимальный вес файлов  (МБ)    (При импортировании
       Максимальный вес файлов (МБ)                        автоматически устанавливаются)
       MD5LOG

     Пример содержимого, составленного Опцией 1:
       C:\\Users\\javas\\OneDrive\\Документы\\Fonts\\Minecraft Rus NEW\\COPYRIGHT.txt
       7079c72c21415131774625ba1d64f4b0
       C:\\Users\\javas\\OneDrive\\Документы\\Fonts\\Minecraft Rus NEW\\LICENSE.txt
       452608796d9cbe7746b83491aaa8000b
       C:\\Users\\javas\\OneDrive\\Документы\\Fonts\\Minecraft Rus NEW\\Minecraft Rus NEW.otf
       62bef77eb4d6d637e8c2dba597a1979e
       0.00047
       0.1328
       MD5LOG""")
...
if __name__ == "__main__":
    ...
    print("(1)Составить контрольные суммы файлов каталога; (2)Найти файлы по составленным данным контрольной суммы; (3)Показать справку; (4)Выйти")
    ...
    while True:
        ...
        while CurrentInput != op[0:2]:
            ...
            elif CurrentInput == op[2]: ec(); showhelp()
            elif CurrentInput == op[3]: ec(); exit()
